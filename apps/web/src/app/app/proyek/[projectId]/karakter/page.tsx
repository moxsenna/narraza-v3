  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6">
      ...
      <ul className="mt-8 space-y-2">
        {characters.length === 0 ? (
          <li className="text-sm text-text-muted">Belum ada karakter.</li>
        ) : (
          characters.map((c) => (
            <li key={c.id} className="rounded-xl border border-border-default bg-surface px-4 py-3">
              <span className="font-bold">{c.displayName}</span>
              <span className="ml-2 text-sm text-text-muted">{c.role}</span>
            </li>
          ))
        )}
      </ul>
