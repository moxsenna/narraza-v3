...
schema.test('1. creates exactly one ledger row with reservation_settlement/debit vocabulary', async ({ databaseUrl }) => {
...
schema.test('2. exact settlement replay returns already_settled without duplicate row', async ({ databaseUrl }) => {
  const prisma = createPrismaClient(databaseUrl);
...
schema.test('3. divergent settlement replay returns binding_invalid conflict', async ({ databaseUrl }) => {
  const prisma = createPrismaClient(databaseUrl);
...
schema.test('5. positive settlement-linked release persists release/credit vocabulary', async ({ databaseUrl }) => {
  const prisma = createPrismaClient(databaseUrl);
...
schema.test('8. zero settlement delta skips INSERT and returns settled', async ({ databaseUrl }) => {
  const prisma = createPrismaClient(databaseUrl);
...
schema.test('19. exact reservation target replay returns already_reconciled without mutation', async ({ databaseUrl }) => {
  ...
  try {
    const res = await seedCreditReservation(prisma, 'closing', 1000000n, 600000n, 100000n, 300000n);
...
describe('Task 8 deriveReservationStatus helper', () => {
  it('verifies deriveReservationStatus produces correct status for all tuple shapes', () => {
    // Case A: E > 0 → closing
    expect(deriveReservationStatus({
      settledTargetMicroIdr: 0n,
      releasedTargetMicroIdr: 0n,
