...
schema.test('proposal lifecycle, hashes, payloads, and tenant parents are constrained', async ({
  client,
}) => {
  await seedUsersAndProjects(client);
...
schema.test('job lease coherence and deferred cross-entity pointers prevent mismatches', async ({
  client,
}) => {
  await seedUsersAndProjects(client);
  await expectSqlState(
...
schema.test('snapshot, workflow plan, attempt, and usage numeric/hash contracts hold', async ({
  client,
}) => {
  await seedUsersAndProjects(client);
