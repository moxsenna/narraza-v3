...
proposalTest('accept-proposal: user prose accept bumps canon once and sets beat pointer', async ({
  prisma,
}) => {
...
proposalTest('accept-cas-stale: base moved → CAS fail, stale mark in new tx', async ({
  prisma,
}) => {
  const { userId, projectId, beatId } = await seedOwnerProjectBeat(prisma);
  const prepared = await prepareUserProposal(
...
proposalTest('accept-supersede: sibling proposals superseded in the same tx', async ({
  prisma,
}) => {
  const { userId, projectId, beatId } = await seedOwnerProjectBeat(prisma);
...
proposalTest('proposal-unrelated-version-bump: unrelated canon bump keeps proposal valid', async ({
  prisma,
}) => {
  const { userId, projectId, beatId } = await seedOwnerProjectBeat(prisma);
  const prepared = await prepareUserProposal(
...
proposalTest('user-proposal: source=user with server-owned hashes and content required', async ({
  prisma,
}) => {
  const { userId, projectId, beatId } = await seedOwnerProjectBeat(prisma);
  ...
  expect(opRows.map((r) => r.operation_type)).toEqual([
    'prose.version.create',
    'prose.accept',
  ]);
...
proposalTest('proposal-operation-hash: tampered ops fail the declared-hash binding', async ({
  prisma,
}) => {
  const { userId, projectId, beatId } = await seedOwnerProjectBeat(prisma);
  const prepared = await prepareUserProposal(
...
proposalTest('publish-artifact: publish accept does not bump canon version', async ({
  prisma,
}) => {
  const { userId, projectId, beatId } = await seedOwnerProjectBeat(prisma);
