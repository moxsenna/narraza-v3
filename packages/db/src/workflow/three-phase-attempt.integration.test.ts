...
suite.test(
  ...
  async ({ client, databaseUrl }) => {
    ...
    expect(
      (await client.query(`SELECT deleted_at FROM projects WHERE id=$1`, [ids.projectA])).rows[0],
...
suite.test(
  ...
  async ({ client, databaseUrl }) => {
    ...
    const prisma = createPrismaForUrl(
...
suite.test(
  ...
  async ({ client, databaseUrl }) => {
    ...
    await expect(harness.run(harnessInput)).rejects.toThrow();
    expect(await durableSnapshot(client)).toMatchObject({
      status: 'running',
      winner_attempt_id: attemptId,
      usage: 1,
